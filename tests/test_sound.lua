...
local function cases()
    ...
    return {
        {
            name = "set_volume accepts both boundaries",
            run = function()
                utils.assert_success(api.set_volume(0))
                utils.assert_success(api.set_volume(100))
            end,
        },
        {
            name = "set_volume accepts an integer string",
            run = function()
                utils.assert_success(api.set_volume("50"))
            end,
        },
        {
            name = "set_volume rejects invalid values",
            run = function()
        ...
        {
            name = "get_volume parses numeric output",
            run = function()
                utils.assert_equal(api.get_volume(), 40, "volume")
            end,
        },
        {
            name = "get_volume rejects malformed output",
            run = function()
        ...
        {
            name = "get_volume propagates backend failure",
            run = function()
        ...
        {
            name = "mute controls succeed",
            run = function()
                utils.assert_success(api.mute())
                utils.assert_success(api.unmute())
                utils.assert_success(api.toggle_mute())
            end,
        },
        {
            name = "is_muted parses false",
            run = function()
                utils.assert_equal(api.is_muted(), false, "mute state")
            end,
        },
        {
            name = "is_muted rejects malformed output",
            run = function()
        ...
        {
            name = "relative volume clamps at both boundaries",
            run = function()
                utils.assert_success(api.increase_volume(100))
                utils.assert_success(api.decrease_volume(100))
            end,
        },
        {
            name = "relative volume rejects invalid amounts",
            run = function()
